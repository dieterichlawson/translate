...
class Text:
  ...
  def __init__(self, text = "text.txt", dictionary='dict.txt', with_rules=True,use_cache=True):
      self.load_text(text)
      self.load_dict(dictionary)
      cache_exists = os.path.isfile(self.translated_file)
      cache_exists = cache_exists and os.path.isfile(self.tagged_file)
      if not use_cache or not cache_exists:
        self.translate(with_rules)
        self.tag()
        self.write_cache()
      else:
        self.load_cache()

  def get_definition_with_rules(self,currIndex,words):
      ...
      if currWord=='dan':
          ...
          if nextWord!='' and self.isVowel(self.get_definition_with_rules(0,[currWord])[0]):
              return 'an'
          else:
              return 'a'
          ...
          return self.get_definitions(currWord)[0]
  def load_cache(self):
    in_translated = open(self.translated_file)
    for line in in_translated:
      line = line.strip()
      self.translated.append(line.split('|'))
    in_translated.close()
    in_tagged = open(self.tagged_file)
    for line in in_tagged:
      taglist = []
      for pair in line.split('|'):
        taglist.append((pair.split('_')[0],pair.split('_')[1]))
      self.tagged.append(taglist)
    in_tagged.close()

  def write_cache(self):
    outfile = open(self.translated_file,'w')
    for sentence in self.translated:
      outfile.write('|'.join(sentence))
      outfile.write('\n')
    outfile.close()
    outfile = open(self.tagged_file,'w')
  ...
  def load_dict(self,filename):
      ...
      for line in dictfile:
          ...
          if '_' in word:
              self.dictionary[word.split('_')[0]].extend(definitions)
      dictfile.close()

  def get_definitions(self,word,pos=None):
      if pos != None and len(self.dictionary[word+'_'+pos]) > 0:
          return self.dictionary[word+'_'+pos]
      else:
          return self.dictionary[word]

  def get_pos(self,word,index,sentence):
      if index > 0 and self.is_personal_pronoun(sentence[index-1]):
          return 'v'
      return None
      ...
      for sentence in self.text:
          ...
          for i,word in enumerate(sentence):
              ...
              if len(word) == 1 and word in punctuation:
                  ...
                  continue
              pos = self.get_pos(word,i,sentence)
              if pos != None: # if we know the POS, pick the best definition
                  trans_word = self.get_definitions(word,pos)[0]
              elif with_rules: # If we're using our special translation rules
                  trans_word = self.get_definition_with_rules(i,sentence)
              else: #we're just doing plain definition grabbing
                  trans_word = self.get_definitions(word)[0]
              trans_sentence.append(trans_word)
          self.translated.append(trans_sentence)

  def tag(self):
    self.tagged = []
    for i,sentence in enumerate(self.translated):
      self.tagged.append(self.tagger.tag(sentence))
      print "Tagged sentence %d of %d" % (i +1,len(self.translated))
